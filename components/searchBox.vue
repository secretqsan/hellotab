...
const handleSearch = () => {
  if (searchQuery.value.trim()) {
    let searchUrl = ''
    if (aiSearch.value) {
      searchUrl = `${aiSearchEngine.value.baseUrl}${encodeURIComponent(searchQuery.value.trim())}`
    }
    else{
...
const fetchSuggestions = async (query) => {
  ...
  try {
    if(!aiSearch.value){
      const response = await $fetch(`/api/proxy/https://api.bing.com/qsonhs.aspx?q=${encodeURIComponent(query)}`)
      ...
      const response = await $fetch(`https://open.bigmodel.cn/api/paas/v4/chat/completions`, {
        ...
        body: {
          model: 'glm-4-flashx',
          messages: [
            {"role": "system", "content": "根据提供的片段给出接下来最合理的几个字，以让整个语句构成一个有意义的简短提问，不要重复我提供的内容"},
    ...
    <div class="relative w-full group">
      <input
        ...
        :class="[
          ...
          aiSearch? 'focus:shadow-[0_0_15px_rgba(255,105,180,0.3),0_0_15px_rgba(147,112,219,0.3)] ': 'focus:border-blue-500 group-focus-within:border-blue-500',
          aiSearch ? 'border-transparent bg-clip-padding bg-[padding-box] border-[2px] border-[transparent]' +
          ' [background-image:linear-gradient(white,white),linear-gradient(to_right,#ff69b4,#9370db)]' +
          ' [background-origin:border-box] [background-clip:padding-box,border-box]' : 'border-slate-200',
      ...
      <button
        ...
        ['absolute right-0 top-1/2 -translate-y-1/2',
          ...
          aiSearch? 'hover:bg-gray-200/50':'bg-blue-400 hover:bg-blue-500 group-focus-within:bg-blue-500',
          showSuggestions && suggestions.length > 0 ? 'rounded-tr-lg' : 'rounded-r-lg',
          'cursor-pointer',
    ...
    <transition 
      ...
      >
        ...
        >
          <div 
            :class="
              'px-4 py-3 cursor-pointer transition-colors duration-200 hover:bg-slate-50'
            "
