...
async function getLocation(){
  try {
    ...
    const res = await $fetch(`/api/weather?type=location&longitude=${location.value.longitude}&latitude=${location.value.latitude}`)
...
async function getWeather() {
  try {
    const res = await $fetch(`/api/weather?type=weather&longitude=${location.value.longitude}&latitude=${location.value.latitude}`)
    if (res.code === '200') {
      weather.value = {
        temp: res.now.temp,
        icon: res.now.icon,
...
<template>
  <WidgetsBase :alt="!weather.temp" :size_x="size_x" :size_y="size_y" :title="'天气'">
    <template #large>
    ...
    <template #middle>
      <div class="h-full w-full flex flex-row items-center justify-center gap-8">
